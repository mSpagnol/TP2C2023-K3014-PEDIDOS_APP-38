...
IF OBJECT_ID ('BI_PEDIDOS_APP.FK_PEDIDO_ETARIO_REPARTIDOR' , 'F') IS NOT NULL
	ALTER TABLE BI_PEDIDOS_APP.BI_HECHO_PEDIDO DROP CONSTRAINT FK_PEDIDO_ETARIO_REPARTIDOR
GO
...
FROM (-- el subselect esta hecho porque no puedo hacer el temas de las fk
    SELECT
        ...
        ID_RANGO_HORARIO, --
		LOCALIDAD_ID AS LOCALIDAD
    FROM BI_PEDIDOS_APP.BI_HECHO_PEDIDO hp
    GROUP BY DATEPART(YEAR, PEDIDO_FECHA), DATEPART(MONTH, PEDIDO_FECHA), ID_DIA, ID_RANGO_HORARIO,LOCALIDAD_ID
) AS Subquery
