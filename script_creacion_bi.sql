...
BEGIN
	...
	TIPO_MEDIO_DE_PAGO_ID)
		...
		e.ENVIO_FECHA_ENTREGA AS PEDIDO_FECHA_ENTREGA,
...
BEGIN
	...
	TIPO_RECLAMO 
		...
		BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_OPERADOR(op.OPERADOR_CODIGO) AS RANGO_ETARIO_OPERADOR_ID,
...
CREATE VIEW BI_PEDIDOS_APP.VISTA_CANTIDAD_PEDIDOS_MES_ANIO_SEGUN_LOCALIDAD AS
WITH CTE AS (
    SELECT 
...
SELECT Localidad, RANGO_HORARIO_DESCRIPCION, DIA_LETRA, Mes, Anio, CATEGORIA_LOCAL, totalPedidos
FROM CTE
WHERE RowNum = 1
GO
...
FROM (
		...
		GROUP BY RANGO_ETARIO_REPARTIDOR, LOCALIDAD_NOMBRE ,TIEMPO_MES
		UNION ALL
