...
BEGIN
	...
	PEDIDO_FECHA_ENTREGA)
        SELECT 
		p.PEDIDO_TOTAL_SERVICIO AS PEDIDO_TOTAL_SERVICIO,
		...
		BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_REPARTIDOR(p.REPARTIDOR_DNI) AS RANGO_ETARIO_REPARTIDOR,
...
FROM (
    SELECT
        ...
        MAX(COUNT(*)) OVER (PARTITION BY t.TIEMPO_ANIO, t.TIEMPO_MES) AS MaxPedidos,
        d.DIA_LETRA AS Dia_Mayor_Pedidos,
        rh.RANGO_HORARIO_DESCRIPCION AS Rango_Horario,
        l.LOCALIDAD_NOMBRE AS Localidad
    FROM BI_PEDIDOS_APP.BI_HECHO_PEDIDO hp
    JOIN BI_PEDIDOS_APP.BI_TIEMPO t ON hp.ID_TIEMPO = t.ID_TIEMPO
    JOIN BI_PEDIDOS_APP.BI_DIA d ON hp.ID_DIA = d.ID_DIA
    JOIN BI_PEDIDOS_APP.BI_RANGO_HORARIO rh ON hp.ID_RANGO_HORARIO = rh.ID_RANGO_HORARIO
    JOIN BI_PEDIDOS_APP.BI_LOCALIDAD l ON hp.LOCALIDAD_ID = l.LOCALIDAD_ID
    GROUP BY t.TIEMPO_ANIO, t.TIEMPO_MES, d.DIA_LETRA, rh.RANGO_HORARIO_DESCRIPCION, l.LOCALIDAD_NOMBRE
) AS Subquery
