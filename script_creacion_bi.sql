...
/* DROP FK */
IF OBJECT_ID ('BI_PEDIDOS_APP.FK_LOCAL_TIPO_ID' , 'F') IS NOT NULL
	ALTER TABLE BI_PEDIDOS_APP.BI_LOCAL DROP CONSTRAINT FK_LOCAL_TIPO_ID
GO

IF OBJECT_ID ('BI_PEDIDOS_APP.FK_PROVINCIA_ID_FK' , 'F') IS NOT NULL
	ALTER TABLE BI_PEDIDOS_APP.BI_LOCALIDAD DROP CONSTRAINT FK_PROVINCIA_ID
GO

IF OBJECT_ID ('BI_PEDIDOS_APP.FK_TIPO_MEDIO_DE_PAGO_ID_FK' , 'F') IS NOT NULL
	ALTER TABLE BI_PEDIDOS_APP.BI_HECHO_PEDIDO DROP CONSTRAINT FK_TIPO_MEDIO_DE_PAGO_ID
GO
...
CREATE TABLE BI_PEDIDOS_APP.BI_HECHO_PEDIDO(
	...
	PROMEDIO_TIEMPO_ESTIMADO_ENTREGA decimal(18,2),
	PROMEDIO_CALIFICACION decimal (18,0),
	DESVIO_PROMEDIO decimal(18,0),
	TOTAL_VALOR_ENVIO decimal(18,2),
...
CREATE TABLE BI_PEDIDOS_APP.BI_HECHO_ENVIO_MENSAJERIA(
	...
	PROMEDIO_PRECIO_SEGURO decimal(18,2),
	PROMEDIO_ENVIO_ENTREGA decimal(18,0),
	PROMEDIO_TIEMPO_ESTIMADO decimal(18,2),
	LOCALIDAD_ID INT,
	ID_DIA INT,
	ID_RANGO_HORARIO INT, 
	ID_TIEMPO INT,
...
CREATE TABLE BI_PEDIDOS_APP.BI_HECHO_RECLAMO(
	NRO_HECHO_RECLAMO INT IDENTITY (1,1) PRIMARY KEY, 
    TOTAL_MONTO_CUPON_ASOCIADO decimal(18,0),
	TIEMPO_PROMEDIO_RESOLUCION decimal(18,0),
	ESTADO_RECLAMO INT,
...
--FK
ALTER TABLE BI_PEDIDOS_APP.BI_HECHO_PEDIDO ADD CONSTRAINT FK_PEDIDO_TIPO_MEDIO_DE_PAGO_ID FOREIGN KEY (TIPO_MEDIO_DE_PAGO_ID) REFERENCES BI_PEDIDOS_APP.BI_TIPO_MEDIO_DE_PAGO(TIPO_MEDIO_DE_PAGO_ID)
GO

ALTER TABLE BI_PEDIDOS_APP.BI_LOCALIDAD ADD CONSTRAINT FK_LOCALIDAD_PROVINCIA_ID FOREIGN KEY (PROVINCIA_ID) REFERENCES BI_PEDIDOS_APP.BI_PROVINCIA(PROVINCIA_ID)
GO

ALTER TABLE BI_PEDIDOS_APP.BI_LOCAL ADD CONSTRAINT FK_LOCAL_LOCAL_TIPO_ID FOREIGN KEY (LOCAL_TIPO_ID) REFERENCES BI_PEDIDOS_APP.BI_LOCAL_TIPO(LOCAL_TIPO_ID)
GO
...
ALTER TABLE BI_PEDIDOS_APP.BI_HECHO_ENVIO_MENSAJERIA ADD CONSTRAINT FK_PAQUETE_NRO FOREIGN KEY (ID_TIPO_PAQUETE) REFERENCES BI_PEDIDOS_APP.BI_TIPO_PAQUETE(PAQUETE_ID)
GO
...
BEGIN
	...
	DECLARE @EDAD INT
	DECLARE @ID_RANGO INT
	SET @FECHA_NAC = (SELECT OPERADOR_FECHA_NAC FROM PEDIDOS_APP.OPERADOR op WHERE op.OPERADOR_CODIGO = @OPERADOR)
...
AS
BEGIN
	INSERT INTO BI_PEDIDOS_APP.BI_LOCALIDAD(LOCALIDAD_ID,LOCALIDAD_NOMBRE,PROVINCIA_ID)
	SELECT DISTINCT l.LOCALIDAD_COD,l.LOCALIDAD_NOMBRE, pl.PROVINCIA_ID
	FROM PEDIDOS_APP.LOCAL_LOCALIDAD l
	join PEDIDOS_APP.LOCAL_PROVINCIA lp on l.PROVINCIA_COD = lp.PROVINCIA_COD
	JOIN BI_PEDIDOS_APP.BI_PROVINCIA pl on  pl.PROVINCIA_NOMBRE = lp.PROVINCIA_NOMBRE
END
GO
...
AS
BEGIN
	INSERT INTO BI_PEDIDOS_APP.BI_LOCAL(LOCAL_NOMBRE, LOCAL_DESCRIPCION, LOCAL_TIPO_ID)
	select l.LOCAL_NOMBRE,l.LOCAL_DESCRIPCION,bt.LOCAL_TIPO_ID
	FROM PEDIDOS_APP.LOCAL l
	join PEDIDOS_APP.LOCAL_TIPO lt on l.LOCAL_TIPO_ID = lt.LOCAL_TIPO_ID 
	join BI_PEDIDOS_APP.BI_LOCAL_TIPO bt on bt.LOCAL_TIPO_DESCRIPCION = lt.LOCAL_TIPO
END
GO
...
BEGIN
	...
	FROM PEDIDOS_APP.LOCAL_CATEGORIA l
END
GO
...
BEGIN
    INSERT INTO BI_PEDIDOS_APP.BI_ESTADO_PEDIDO (PEDIDO_ESTADO)
        ...
        FROM PEDIDOS_APP.PEDIDO_ESTADO p
END
GO
...
AS
BEGIN
    INSERT INTO BI_PEDIDOS_APP.BI_HECHO_PEDIDO(
	PEDIDO_TOTAL_SERVICIO,
	PEDIDO_TOTAL_CUPONES,
	DESVIO_PROMEDIO,
	PROMEDIO_CALIFICACION,
	PROMEDIO_TIEMPO_ESTIMADO_ENTREGA,
	TOTAL_VALOR_ENVIO,
	LOCAL_NOMBRE,
	...
	CANTIDAD_PEDIDOS)
        SELECT 
		SUM(p.PEDIDO_TOTAL_SERVICIO) AS PEDIDO_TOTAL_SERVICIO,
		SUM(p.PEDIDO_TOTAL_CUPONES) AS PEDIDO_TOTAL_CUPONES,
		AVG(DATEDIFF(MINUTE, p.PEDIDO_FECHA, e.ENVIO_FECHA_ENTREGA)),
		AVG(p.PEDIDO_CALIFICACION),
		AVG(p.PEDIDO_TIEMPO_ESTIMADO_ENTREGA),
		SUM(e.ENVIO_PRECIO),
		...
		d.ID_DIA,
        rh.ID_RANGO_HORARIO,
        lc.LOCALIDAD_ID AS LOCALIDAD,
		cl.LOCAL_CATEGORIA_ID AS CATEGORIA_LOCAL,
		...
		join BI_PEDIDOS_APP.BI_LOCALIDAD lc on lc.LOCALIDAD_ID = dl.LOCALIDAD_COD 
		...
		join PEDIDOS_APP.MEDIO_PAGO mp on mp.MEDIO_PAGO_NRO_TARJETA = p.MEDIO_DE_PAGO_NRO_TARJETA
		join BI_PEDIDOS_APP.BI_TIPO_MEDIO_DE_PAGO tmp on mp.MEDIO_PAGO_TIPO = tmp.TIPO_MEDIO_DE_PAGO
		join BI_PEDIDOS_APP.BI_RANGO_ETARIO ru on ru.RANGO_ETARIO_ID = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_USUARIO(p.USUARIO_DNI)
		join BI_PEDIDOS_APP.BI_RANGO_ETARIO rp on rp.RANGO_ETARIO_ID = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_REPARTIDOR(e.REPARTIDOR_DNI)
		join PEDIDOS_APP.PEDIDO_ESTADO pe on pe.PEDIDO_ESTADO_ID = p.PEDIDO_ESTADO_ID	
		join BI_PEDIDOS_APP.BI_ESTADO_PEDIDO ep on ep.PEDIDO_ESTADO = pe.PEDIDO_ESTADO
		join BI_PEDIDOS_APP.BI_DIA d on d.ID_DIA = DATEPART(WEEKDAY, p.PEDIDO_FECHA)
		join BI_PEDIDOS_APP.BI_RANGO_HORARIO rh on rh.ID_RANGO_HORARIO = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_HORARIO(DATEPART(HOUR, p.PEDIDO_FECHA))
		join BI_PEDIDOS_APP.BI_TIPO_MOVILIDAD tm on tm.TIPO_MOVILIDAD_ID = r.TIPO_MOVILIDAD_COD
		join PEDIDOS_APP.LOCAL_CATEGORIA lcat on l.LOCAL_CATEGORIA_ID = lcat.LOCAL_CATEGORIA_ID
		join BI_PEDIDOS_APP.BI_LOCAL_CATEGORIA cl on cl.LOCAL_CATEGORIA = lcat.LOCAL_CATEGORIA

		GROUP BY p.LOCAL_NOMBRE, ru.RANGO_ETARIO_ID, rp.RANGO_ETARIO_ID, ep.ESTADO_PEDIDO_ID, d.ID_DIA, rh.ID_RANGO_HORARIO, lc.LOCALIDAD_ID,
        l.LOCAL_CATEGORIA_ID, t.ID_TIEMPO, tm.TIPO_MOVILIDAD_ID, tmp.TIPO_MEDIO_DE_PAGO_ID,cl.LOCAL_CATEGORIA_ID
END
GO
...
AS
	BEGIN 
		INSERT INTO BI_PEDIDOS_APP.BI_HECHO_ENVIO_MENSAJERIA(
			PROMEDIO_ENVIO_ENTREGA,
			PROMEDIO_PRECIO_SEGURO,
			PROMEDIO_VALOR_ASEGURADO,
			PROMEDIO_TIEMPO_ESTIMADO,
			LOCALIDAD_ID,
			ID_DIA,
		...
		SELECT 
			AVG(DATEDIFF(MINUTE, e.ENVIO_MENSAJERIA_FECHA, e.ENVIO_MENSAJERIA_ENTREGA)),
			AVG(e.ENVIO_MENSAJERIA_PRECIO_SEGURO),
			AVG(e.ENVIO_MENSAJERIA_VALOR_ASEGURADO),
			AVG(e.ENVIO_MENSAJERIA_TIEMPO_ESTIMADO),
			l.LOCALIDAD_ID,
			d.ID_DIA,
			...
			JOIN BI_PEDIDOS_APP.BI_DIA d on d.ID_DIA = DATEPART(WEEKDAY, e.ENVIO_MENSAJERIA_FECHA)
			...
			join PEDIDOS_APP.REPARTIDOR r on e.REPARTIDOR_DNI = r.REPARTIDOR_DNI
			join BI_PEDIDOS_APP.BI_RANGO_HORARIO rh on rh.ID_RANGO_HORARIO = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_HORARIO(DATEPART(HOUR, e.ENVIO_MENSAJERIA_FECHA))
			join BI_PEDIDOS_APP.BI_RANGO_ETARIO er on er.RANGO_ETARIO_ID = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_REPARTIDOR(e.REPARTIDOR_DNI)
			join BI_PEDIDOS_APP.BI_RANGO_ETARIO ur on ur.RANGO_ETARIO_ID = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_USUARIO(e.USUARIO_DNI)
			GROUP BY l.LOCALIDAD_ID, d.ID_DIA, es.ESTADO_ENVIO_MENSAJERIA_ID, rh.ID_RANGO_HORARIO, t.ID_TIEMPO, er.RANGO_ETARIO_ID,
			ur.RANGO_ETARIO_ID, tp.PAQUETE_ID, r.TIPO_MOVILIDAD_COD
	END
GO
-------------------
---------------------
...
BEGIN
	...
	CANTIDAD_RECLAMOS 
        ...
        SELECT
		SUM(c.CUPON_RECLAMO_MONTO) AS TOTAL_MONTO_CUPON_ASOCIADO,
		AVG(DATEDIFF(MINUTE,r.RECLAMO_FECHA, r.RECLAMO_FECHA_SOLUCION)),
		er.ESTADO_RECLAMO_ID,
		re.RANGO_ETARIO_ID,
		rh.ID_RANGO_HORARIO,
		t.ID_TIEMPO,
		d.ID_DIA,
		...
		join BI_PEDIDOS_APP.BI_DIA d on DATEPART(WEEKDAY, r.RECLAMO_FECHA) = d.ID_DIA
		join PEDIDOS_APP.CUPON_RECLAMO c on r.RECLAMO_NRO = c.RECLAMO_NRO
		JOIN BI_PEDIDOS_APP.BI_ESTADO_RECLAMO er on er.ESTADO_RECLAMO_ID = BI_PEDIDOS_APP.FX_CALCULAR_ESTADO(r.RECLAMO_ESTADO)
		JOIN BI_PEDIDOS_APP.BI_RANGO_ETARIO re on re.RANGO_ETARIO_ID = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_ETARIO_OPERADOR(op.OPERADOR_CODIGO)
		JOIN BI_PEDIDOS_APP.BI_RANGO_HORARIO rh on rh.ID_RANGO_HORARIO = BI_PEDIDOS_APP.FX_CALCULAR_RANGO_HORARIO(DATEPART(HOUR, r.RECLAMO_FECHA))
		GROUP BY er.ESTADO_RECLAMO_ID, re.RANGO_ETARIO_ID, rh.ID_RANGO_HORARIO, t.ID_TIEMPO, d.ID_DIA, r.RECLAMO_TIPO
END
GO
...
AS
	...
	FROM (
		SELECT
			...
			ROW_NUMBER() OVER (PARTITION BY t.TIEMPO_ANIO, t.TIEMPO_MES, l.LOCALIDAD_NOMBRE, d.DIA_LETRA ORDER BY COUNT(*) DESC) AS RowNumber
		FROM BI_PEDIDOS_APP.BI_HECHO_PEDIDO p
		JOIN BI_PEDIDOS_APP.BI_TIEMPO t ON t.ID_TIEMPO = p.ID_TIEMPO
		JOIN BI_PEDIDOS_APP.BI_RANGO_HORARIO rh ON rh.ID_RANGO_HORARIO = p.ID_RANGO_HORARIO
		JOIN BI_PEDIDOS_APP.BI_LOCALIDAD l ON l.LOCALIDAD_ID = p.LOCALIDAD_ID
		JOIN BI_PEDIDOS_APP.BI_DIA d ON d.ID_DIA = p.ID_DIA
		GROUP BY t.TIEMPO_ANIO, t.TIEMPO_MES, rh.RANGO_HORARIO_DESCRIPCION, p.CATEGORIA_LOCAL, l.LOCALIDAD_NOMBRE, d.DIA_LETRA
	) AS Subquery
	WHERE RowNumber = 1
GO
...
AS
	SELECT
		l.LOCALIDAD_NOMBRE,
		AVG(p.TOTAL_VALOR_ENVIO) as PROMEDIO_MENSUAL,
		t.TIEMPO_MES as Mes,
...
FROM (
	...
	WHERE hp.DESVIO_PROMEDIO IS NOT NULL
    ...
    SELECT 
        ...
        tp.TIPO_MOVILIDAD_VEHICULO,
        d.DIA_LETRA,
        em.PROMEDIO_ENVIO_ENTREGA as desvio
	...
	WHERE em.PROMEDIO_ENVIO_ENTREGA IS NOT NULL
...
AS
		SELECT
			hp.LOCAL_NOMBRE,
			hp.PROMEDIO_CALIFICACION as CALIFICACION_PROMEDIO,
			t.TIEMPO_MES,
			t.TIEMPO_ANIO --- pide la mensual pero no dice que por año asi que capaz este lo borramos
		FROM BI_PEDIDOS_APP.BI_HECHO_PEDIDO hp
		join BI_PEDIDOS_APP.BI_TIEMPO t on t.ID_TIEMPO = hp.ID_TIEMPO
	GROUP BY hp.LOCAL_NOMBRE, TIEMPO_ANIO, TIEMPO_MES, hp.PROMEDIO_CALIFICACION
GO
...
AS
	SELECT 
		...
		LOCALIDAD_NOMBRE,
		TIEMPO_MES,
		SUM(COUNT(*)) OVER (PARTITION BY TIEMPO_MES) AS TotalVerdadero,
		SUM(cantidad) as cantidad_esperada,
		sum(cantidad) * 100.0 / 
		SUM(COUNT(*)) OVER (PARTITION BY TIEMPO_MES)  as porcentaje_cantidad
FROM (
		SELECT
			...
			hp.CANTIDAD_PEDIDOS AS cantidad
		FROM BI_PEDIDOS_APP.BI_HECHO_PEDIDO hp
		join BI_PEDIDOS_APP.BI_TIEMPO t on t.ID_TIEMPO = hp.ID_TIEMPO
		JOIN BI_PEDIDOS_APP.BI_LOCALIDAD l on l.LOCALIDAD_ID = hp.LOCALIDAD_ID 
		GROUP BY RANGO_ETARIO_REPARTIDOR, LOCALIDAD_NOMBRE ,TIEMPO_MES, hp.CANTIDAD_PEDIDOS
		...
		GROUP BY RANGO_ETARIO_REPARTIDOR, LOCALIDAD_NOMBRE ,TIEMPO_MES, em.CANTIDAD_ENVIOS
)AS Subquery
	GROUP BY RANGO_ETARIO_REPARTIDOR, cantidad,LOCALIDAD_NOMBRE ,TIEMPO_MES
GO
...
AS
	SELECT
			tp.PAQUETE_TIPO,
			avg(em.PROMEDIO_VALOR_ASEGURADO) as PROMEDIO_VALOR_ASEUGRADO,
			t.TIEMPO_MES
		FROM BI_PEDIDOS_APP.BI_HECHO_ENVIO_MENSAJERIA em
		join BI_PEDIDOS_APP.BI_TIPO_PAQUETE tp on tp.PAQUETE_ID = em.ID_TIPO_PAQUETE
		join BI_PEDIDOS_APP.BI_TIEMPO t on t.ID_TIEMPO = em.ID_TIEMPO
...
AS
	SELECT
		r.TIPO_RECLAMO,
		re.RANGO_ETARIO_DESCRIPCION,
		AVG(r.TIEMPO_PROMEDIO_RESOLUCION) AS TIEMPO_PROMEDIO_RESOLUCION
	FROM BI_PEDIDOS_APP.BI_HECHO_RECLAMO r
...
AS
	SELECT
		SUM(r.TOTAL_MONTO_CUPON_ASOCIADO) as MONTO_MENSUAL_CUPONES,
		t.TIEMPO_ANIO,
		t.TIEMPO_MES
	FROM BI_PEDIDOS_APP.BI_HECHO_RECLAMO r
	JOIN BI_PEDIDOS_APP.BI_TIEMPO t on t.ID_TIEMPO = r.ID_TIEMPO
...
SELECT * FROM BI_PEDIDOS_APP.BI_HECHO_ENVIO_MENSAJERIA
SELECT * FROM BI_PEDIDOS_APP.BI_HECHO_PEDIDO
SELECT * FROM BI_PEDIDOS_APP.BI_HECHO_RECLAMO