...
BEGIN
	INSERT INTO PEDIDOS_APP.LOCAL 
		(LOCAL_NOMBRE, LOCAL_DESCRIPCION, LOCAL_TIPO)
...
BEGIN
	INSERT INTO PEDIDOS_APP.PEDIDO
		...
		SELECT DISTINCT m.PEDIDO_NRO, m.PEDIDO_TOTAL_PRODUCTOS, m.PEDIDO_TARIFA_SERVICIO,m.PEDIDO_TOTAL_CUPONES, m.PEDIDO_TOTAL_SERVICIO,
						m.PEDIDO_OBSERV, m.PEDIDO_FECHA,m.PEDIDO_TIEMPO_ESTIMADO_ENTREGA, m.PEDIDO_CALIFICACION, mp.MEDIO_PAGO_NRO_TARJETA, l.LOCAL_NOMBRE, u.USUARIO_DNI, m.PEDIDO_ESTADO
		FROM gd_esquema.Maestra m
		JOIN USUARIO u on u.USUARIO_DNI = m.USUARIO_DNI
...
SELECT 
	...
	p.PEDIDO_ESTADO
FROM PEDIDOS_APP.ENVIO e
JOIN PEDIDOS_APP.PEDIDO p on p.PEDIDO_NRO = e.PEDIDO_NRO
