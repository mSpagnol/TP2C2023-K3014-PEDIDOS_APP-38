...
CREATE TABLE PEDIDOS_APP.LOCAL(
	LOCAL_CODIGO INT IDENTITY PRIMARY KEY , 
...
BEGIN
	INSERT INTO PEDIDOS_APP.USUARIO 
		(USUARIO_DNI, USUARIO_NOMBRE, USUARIO_APELLIDO, USUARIO_FECHA_REGISTRO, USUARIO_TELEFONO, USUARIO_MAIL, USUARIO_FECHA_NAC, MEDIO_DE_PAGO_NRO_TARJETA)
	SELECT DISTINCT m.USUARIO_DNI, m.USUARIO_NOMBRE, m.USUARIO_APELLIDO, m.USUARIO_FECHA_REGISTRO, m.USUARIO_TELEFONO, m.USUARIO_MAIL, m.USUARIO_FECHA_NAC, mp.MEDIO_PAGO_NRO_TARJETA
	FROM GD_ESQUEMA.MAESTRA m
	JOIN PEDIDOS_APP.MEDIO_PAGO mp ON (mp.USUARIO_DNI = m.USUARIO_DNI)
...
BEGIN
	...
	FROM gd_esquema.Maestra m
	JOIN USUARIO u on u.MEDIO_DE_PAGO_NRO_TARJETA = m.MEDIO_PAGO_NRO_TARJETA
	where MEDIO_DE_PAGO_NRO_TARJETA is not null
END
GO
...
BEGIN
	INSERT INTO PEDIDOS_APP.USUARIO_DIRECCION
		...
		SELECT DISTINCT m.DIRECCION_USUARIO_DIRECCION, m.DIRECCION_USUARIO_LOCALIDAD, m.DIRECCION_USUARIO_PROVINCIA, u.USUARIO_DNI
		FROM gd_esquema.Maestra m
		JOIN USUARIO u on u.USUARIO_NOMBRE = m.DIRECCION_USUARIO_NOMBRE 
...
BEGIN
	INSERT INTO PEDIDOS_APP.PRODUCTO_LOCAL
		...
		SELECT DISTINCT m.PRODUCTO_LOCAL_CODIGO, PRODUCTO_LOCAL_DESCRIPCION, m.PRODUCTO_LOCAL_PRECIO, m.PRODUCTO_LOCAL_NOMBRE
		FROM gd_esquema.Maestra m
...
BEGIN
	INSERT INTO PEDIDOS_APP.LOCAL 
		(LOCAL_NOMBRE, LOCAL_DESCRIPCION, LOCAL_TIPO, LOCAL_DIRECCION, LOCAL_LOCALIDAD, LOCAL_PROVINCIA)
		SELECT DISTINCT m.LOCAL_NOMBRE, m.LOCAL_DESCRIPCION, m.LOCAL_TIPO, m.LOCAL_DIRECCION, m.LOCAL_LOCALIDAD, m.LOCAL_PROVINCIA
		FROM gd_esquema.Maestra m
...
BEGIN
	INSERT INTO PEDIDOS_APP.PEDIDO
		...
		PEDIDO_TOTAL_CUPONES, PEDIDO_TOTAL_SERVICIO, PEDIDO_OBSERV, PEDIDO_FECHA, PEDIDO_FECHA_ENTREGA, PEDIDO_TIEMPO_ESTIMADO_ENTREGA, PEDIDO_CALIFICACION, MEDIO_DE_PAGO_NRO_TARJETA, LOCAL_CODIGO,
		USUARIO_DNI)
		SELECT DISTINCT m.PEDIDO_NRO, m.PEDIDO_TOTAL_PRODUCTOS, m.PEDIDO_TARIFA_SERVICIO,m.PEDIDO_TOTAL_CUPONES, m.PEDIDO_TOTAL_SERVICIO,
						m.PEDIDO_OBSERV, m.PEDIDO_FECHA, m.PEDIDO_FECHA_ENTREGA, m.PEDIDO_TIEMPO_ESTIMADO_ENTREGA, m.PEDIDO_CALIFICACION, mp.MEDIO_PAGO_NRO_TARJETA, l.LOCAL_CODIGO, u.USUARIO_DNI
		FROM gd_esquema.Maestra m
...
BEGIN
	INSERT INTO PEDIDOS_APP.ENVIO
		(ENVIO_DIRECCION, ENVIO_PRECIO, ENVIO_PROPINA, PEDIDO_NRO, REPARTIDOR_DNI)
		SELECT DISTINCT ud.USUARIO_DIRECCION, m.PEDIDO_PRECIO_ENVIO, m.PEDIDO_PROPINA, p.PEDIDO_NRO, r.REPARTIDOR_DNI
		FROM gd_esquema.Maestra m
...
BEGIN
	INSERT INTO PEDIDOS_APP.OPERADOR
		...
		SELECT DISTINCT m.OPERADOR_RECLAMO_DNI, m.OPERADOR_RECLAMO_NOMBRE, m.OPERADOR_RECLAMO_FECHA_NAC, m.OPERADOR_RECLAMO_MAIL, m.OPERADOR_RECLAMO_TELEFONO
		FROM gd_esquema.Maestra m
...
BEGIN 
	INSERT INTO PEDIDOS_APP.CUPON
		...
		SELECT DISTINCT m.CUPON_NRO, m.CUPON_MONTO, m.CUPON_FECHA_ALTA, m.CUPON_FECHA_VENCIMIENTO, m.CUPON_TIPO
		FROM gd_esquema.Maestra m
...
BEGIN
	INSERT INTO PEDIDOS_APP.RECLAMO
		(RECLAMO_NRO, RECLAMO_CALIFICACION, RECLAMO_DESCRIPCION, RECLAMO_ESTADO, RECLAMO_FECHA, RECLAMO_SOLUCION, RECLAMO_TIPO, PEDIDO_NRO, USUARIO_DNI)
		SELECT DISTINCT m.RECLAMO_NRO, m.RECLAMO_CALIFICACION, m.RECLAMO_DESCRIPCION, m.RECLAMO_ESTADO, m.RECLAMO_FECHA, m.RECLAMO_SOLUCION, m.RECLAMO_TIPO, p.PEDIDO_NRO, u.USUARIO_DNI
		FROM gd_esquema.Maestra m
		JOIN PEDIDO p on p.PEDIDO_NRO = m.PEDIDO_NRO
		JOIN USUARIO u on u.USUARIO_DNI = m.USUARIO_DNI
...
BEGIN
	INSERT INTO PEDIDOS_APP.ENVIO_MENSAJERIA
		...
		ENVIO_MENSAJERIA_LOCALIDAD, ENVIO_MENSAJERIA_OBSERV, ENVIO_MENSAJERIA_PRECIO_ENVIO, ENVIO_MENSAJERIA_PRECIO_SEGURO, ENVIO_MENSAJERIA_PROPINA, ENVIO_MENSAJERIA_PROPINA, ENVIO_MENSAJERIA_PROVINCIA,
		ENVIO_MENSAJERIA_TIEMPO_ESTIMADO, ENVIO_MENSAJERIA_TOTAL, ENVIO_MENSAJERIA_VALOR_ASEGURADO, MEDIO_PAGO_NRO_TARJETA, PAQUETE_NRO, USUARIO_DNI, REPARTIDOR_DNI)
		SELECT DISTINCT m.ENVIO_MENSAJERIA_NRO, m.ENVIO_MENSAJERIA_CALIFICACION, m.ENVIO_MENSAJERIA_DIR_DEST, m.ENVIO_MENSAJERIA_DIR_ORIG, m.ENVIO_MENSAJERIA_FECHA_ENTREGA, m.ENVIO_MENSAJERIA_FECHA,
						...
						u.USUARIO_DNI, r.REPARTIDOR_DNI
		FROM gd_esquema.Maestra m 
		JOIN USUARIO u on u.USUARIO_DNI = m.USUARIO_DNI
		JOIN MEDIO_PAGO mp on mp.USUARIO_DNI = u.USUARIO_DNI
		JOIN REPARTIDOR r on r.REPARTIDOR_DNI = m.REPARTIDOR_DNI
